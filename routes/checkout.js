    ...
    switch (event.type) {
      case 'checkout.session.completed':
        ...
        try {
          ...
          if (order) {
            console.log('✅ Bestellung aktualisiert, E-Mail wird gesendet. - Checkout');
            await sendGameLink(order.email, session.id,session.metadata.gameId, game.name, order.price);
          } else {
